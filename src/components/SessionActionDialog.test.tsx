...
const bridge = vi.hoisted(() => ({ renameThread: vi.fn(), archiveThread: vi.fn(), unarchiveThread: vi.fn() }));
...
describe("SessionActionDialog", () => {
  it("renames a session", async () => {
    ...
    render(<SessionActionDialog action="rename" session={session} onClose={vi.fn()} onRenamed={onRenamed} onArchived={vi.fn()} onRestored={vi.fn()} />);
  ...
  it("archives a session after confirmation", async () => {
    ...
    render(<SessionActionDialog action="archive" session={session} onClose={vi.fn()} onRenamed={vi.fn()} onArchived={onArchived} onRestored={vi.fn()} />);
  ...
  it("restores an archived session", async () => {
    bridge.unarchiveThread.mockResolvedValue(session);
    const onRestored = vi.fn();
    render(<SessionActionDialog action="restore" session={session} onClose={vi.fn()} onRenamed={vi.fn()} onArchived={vi.fn()} onRestored={onRestored} />);

    fireEvent.click(screen.getByRole("button", { name: "Restore" }));

    await waitFor(() => expect(bridge.unarchiveThread).toHaveBeenCalledWith("thread-1"));
    expect(onRestored).toHaveBeenCalledWith(session);
  });
});
